...
export class CurveSpellEth implements Solver {
  ...
  async solve({
    ...
    const curveSwap = CurveFi__factory.connect(this.curveSwapAddress, multicallSwapperSigner);
    ...
    const addLiquidityTx = await curveSwap.populateTransaction['add_liquidity(uint256[2],uint256,bool,address)'](
      [wethBalance, 0],
