...
export class ThreePoolCrv implements Solver {
  ...
  async solve({
    ...
    const crv3Pool = CurveFi__factory.connect(this.crv3PoolAddress, multicallSwapperSigner);
    ...
    const yvBoostVault = YvBoostVault__factory.connect(this.yvBoostAddress, multicallSwapperSigner);
    ...
    const removeLiquidityTx = await crv3Pool.populateTransaction['remove_liquidity_one_coin(uint256,uint256,uint256)'](amount, 1, 0);
    ...
    const yvBoostWithdrawTx = await yvBoostVault.populateTransaction['withdraw(uint256,address,uint256)'](
